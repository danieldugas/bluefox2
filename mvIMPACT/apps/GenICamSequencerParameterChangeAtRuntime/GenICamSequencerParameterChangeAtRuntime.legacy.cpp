...
static SequencerSetParameter s_SequencerLoopData[] =
{
...
//-----------------------------------------------------------------------------
{
    cout << "#############################################################################" << endl
         << "#############################################################################" << endl
         << "# This example runs " << sizeof( s_SequencerData ) / sizeof( s_SequencerData[0] ) << " SequencerSets, where the ExposureTime varies between" << endl
...
//-----------------------------------------------------------------------------
{
    try
    {
        ...
        pTP->sc.sequencerFeatureEnable.write( bTrue );
        const size_t sequencerDataSize = sizeof( s_SequencerData ) / sizeof( s_SequencerData[0] );
        for( size_t i = 0; i < sequencerDataSize; i++ )
        {
            configureSequencerSet( pTP, s_SequencerData[i] );
        }
        const size_t sequencerLoopDataSize = sizeof( s_SequencerLoopData ) / sizeof( s_SequencerLoopData[0] );
        for( size_t i = 0; i < sequencerLoopDataSize; i++ )
        {
            configureSequencerSet( pTP, s_SequencerLoopData[i] );
        }
...
//-----------------------------------------------------------------------------
{
    try
    {
        ...
        if( pTP->runRandomParameters )
        {
            ...
            int sequencerSetNumber = rand() % sequencerDataSize;
