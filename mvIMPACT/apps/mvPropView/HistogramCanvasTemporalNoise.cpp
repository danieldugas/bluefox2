        ...
        ( m_lastImage.getBuffer()->iHeight > 0 ) )
    {
        ...
        case ibpfMono8:
            if( m_bayerParity == bmpUndefined )
            {
                for( int line = 0; line < m_AOIh; line++ )
                {
                    ...
                    {
                        const unsigned char difference = static_cast<unsigned char>( abs( pDataNew[pixel] - pDataOld[pixel] ) );
        ...
        case ibpfMono16:
            if( m_bayerParity == bmpUndefined )
            {
                for( int line = 0; line < m_AOIh; line++ )
                {
                    ...
                    for( int pixel = 0; pixel < m_AOIw; pixel++ )
                    {
                        const unsigned short value = saveAssign( static_cast<unsigned short>( abs( pDataNew[pixel] - pDataOld[pixel] ) ), static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) );
        ...
        case ibpfBGR101010Packed_V2:
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    ...
                    unsigned short difference = static_cast<unsigned short>( abs( blueNew - blueOld ) );
                    m_pHistogramAverage[pBlue] += difference;
                    ++m_ppHistogramBuffer[pBlue][difference];
                    difference = static_cast<unsigned short>( abs( greenNew - greenOld ) );
                    m_pHistogramAverage[pGreen] += difference;
                    ++m_ppHistogramBuffer[pGreen][difference];
                    difference = static_cast<unsigned short>( abs( redNew - redOld ) );
                    m_pHistogramAverage[pRed] += difference;
                    ++m_ppHistogramBuffer[pRed][difference];
                }
            }
        ...
        case ibpfRGBx888Planar:
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    unsigned char difference = static_cast<unsigned char>( abs( *prNew++ - *prOld++ ) );
                    m_pHistogramAverage[pRed] += difference;
                    ++m_ppHistogramBuffer[pRed][difference];
                    difference = static_cast<unsigned char>( abs( *pgNew++ - *pgOld++ ) );
                    m_pHistogramAverage[pGreen] += difference;
                    ++m_ppHistogramBuffer[pGreen][difference];
                    difference = static_cast<unsigned char>( abs( *pbNew++ - *pbOld++ ) );
                    m_pHistogramAverage[pBlue] += difference;
                    ++m_ppHistogramBuffer[pBlue][difference];
                }
            }
        ...
        case ibpfYUV422Planar:
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    unsigned char difference = static_cast<unsigned char>( abs( *pyNew++ - *pyOld++ ) );
                    m_pHistogramAverage[pY] += difference;
                    ++m_ppHistogramBuffer[pY][difference];
                    difference = static_cast<unsigned char>( abs( *puNew - *puOld ) );
                    m_pHistogramAverage[pU] += difference;
                    ++m_ppHistogramBuffer[pU][difference];
                    difference = static_cast<unsigned char>( abs( *pvNew - *pvOld ) );
...
//-----------------------------------------------------------------------------
void HistogramCanvasTemporalNoise::CalculateTemporalNoiseHistogram_8u_RGBPacked( const ImageBuffer* pIBNew, const ImageBuffer* pIBOld, const int order[3] )
//-----------------------------------------------------------------------------
{
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = 0; pixel < m_AOIw; pixel++ )
        {
            unsigned char difference = static_cast<unsigned char>( abs( pDataNew[order[0]] - pDataOld[order[0]] ) );
            m_pHistogramAverage[pBlue] += difference;
            ++m_ppHistogramBuffer[pBlue][difference];
            difference = static_cast<unsigned char>( abs( pDataNew[order[1]] - pDataOld[order[1]] ) );
            m_pHistogramAverage[pGreen] += difference;
            ++m_ppHistogramBuffer[pGreen][difference];
            difference = static_cast<unsigned char>( abs( pDataNew[order[2]] - pDataOld[order[2]] ) );
            m_pHistogramAverage[pRed] += difference;
            ++m_ppHistogramBuffer[pRed][difference];
...
//-----------------------------------------------------------------------------
{
    if( m_bayerParity == bmpUndefined )
    {
        for( int line = 0; line < m_AOIh; line++ )
        {
            ...
            for( int pixel = 0; pixel < m_AOIw; pixel++ )
            {
                const unsigned short valueNew = pixelAccessFn( static_cast<unsigned char*>( pIBNew->vpData ), pixOffset + m_AOIx + pixel );
                const unsigned short valueOld = pixelAccessFn( static_cast<unsigned char*>( pIBOld->vpData ), pixOffset + m_AOIx + pixel );
                const unsigned short difference = static_cast<unsigned short>( abs( valueNew - valueOld ) );
                m_pHistogramAverage[pRed] += difference;
                ++m_ppHistogramBuffer[pRed][difference];
            }
        }
...
//-----------------------------------------------------------------------------
{
    ImageBufferDesc tmp( channelCount );
    tmp.getBuffer()->pixelFormat = ibpfMono8;
    tmp.getBuffer()->iWidth = 0;
    tmp.getBuffer()->iHeight = 0;
    m_lastImage = tmp;
}
