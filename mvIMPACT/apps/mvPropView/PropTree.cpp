...
//-----------------------------------------------------------------------------
{
    wxWindowUpdateLocker updateLock( m_pPropGridPage->GetGrid() );
    CompToDataMap::iterator it = m_CompToDataMap.begin();
    CompToDataMap::iterator itEnd = m_CompToDataMap.end();
    while( it != itEnd )
    {
        it->second->GetGridItem()->SetClientData( 0 );
        delete it->second;
        ++it;
    }
    m_CompToDataMap.clear();
    m_DataToCompMap.clear();
    Delete();
}
...
//-----------------------------------------------------------------------------
{
    if( m_pTopLevelProp )
    {
        ...
        {
            Component it( pPropData->GetComponent().firstChild() );
...
//-----------------------------------------------------------------------------
{
    if( m_rootList.isValid() )
    {
        ...
        if( m_pTopLevelProp )
        {
            ...
            UpdateGridPropsRecursively( Component( m_rootList.firstChild() ), m_pTopLevelProp, boForceRedraw );
        }
        else
        {
...
void PropTree::UpdateGridPropsRecursively( Component iter, wxPGProperty* pListRoot, bool boForceRedraw ) const
//-----------------------------------------------------------------------------
{
    while( iter.isValid() )
    {
...
//-----------------------------------------------------------------------------
{
    while( pListRoot )
    {
        ...
        if( pPropData )
        {
            ...
            Component c( pPropData->GetComponent() );
            if( c.isValid() )
            {
                ...
                if( propertyName != fullFeaturePath )
                {
                    ...
                    if( c.selectingFeatureCount() > 0 )
                    {
                        // The wxPropertyGrid will return the names of children under a 'normal' property as 'parentName.propertyName'!
            ...
            if( boForceDelete || !c.isValid() )
            {
                ...
                {
                    CompToDataMap::iterator itCompToData = m_CompToDataMap.find( c.hObj() );
            ...
            else if( ( c.isList() && ( pPropData->HasChanged() || boForceFullIteration ) ) ||
                     ( ( m_flags & dfSelectorGrouping ) && ( c.selectedFeatureCount() > 0 ) && ( pListRoot->GetChildCount() > 0 ) ) )
            {
        ...
        const size_t nextIndex = pListRoot->GetIndexInParent() + 1;
        pListRoot = nextIndex < static_cast<size_t>( parent->GetChildCount() ) ? parent->Item( static_cast<unsigned int>( nextIndex ) ) : wxNullProperty;
        if( toDelete )
        {
