...
//-----------------------------------------------------------------------------
inline unsigned long GetIPv4AddressFromString( const string& value )
//-----------------------------------------------------------------------------
{
    in_addr IPv4Addr;
    inet_pton( AF_INET, value.c_str(), &IPv4Addr );
#if defined(linux) || defined(__linux) || defined(__linux__)
    return IPv4Addr.s_addr;
#else
    return IPv4Addr.S_un.S_addr;
#endif // #if defined(linux) || defined(__linux) || defined(__linux__)
}
...
//-----------------------------------------------------------------------------
{
    wxString fullFeaturePath( ConvertedString( m_Component.name() ) );
    Component cit( m_Component );
    while( cit.hObj() != m_rootList )
    {
        cit = cit.parent();
        if( !cit.isValid() )
        {
...
//-----------------------------------------------------------------------------
bool PropData::IsVisible( Component c )
//-----------------------------------------------------------------------------
{
    return ( c.isVisible() &&
             ( !GlobalDataStorage::Instance()->IsComponentVisibilitySupported() || ( c.visibility() <= GlobalDataStorage::Instance()->GetComponentVisibility() ) ) );
}
...
//-----------------------------------------------------------------------------
{
    ...
    if( boUpdateNow )
    {
        ...
        {
            const bool boIsVisible = IsVisible();
            const bool boIsList = GetComponent().isList();
            const bool boGridPropertyIsHidden = m_GridItemId->HasFlag( wxPG_PROP_HIDDEN ) != 0;
            if( boIsList )
            {
                if( IsVisible( GetComponent() ) )
                {
                    bool hasVisibleChildren = false;
                    Component iter( GetComponent().firstChild() );
                    while( iter.isValid() )
                    {
                        if( IsVisible( iter ) )
                        {
                    ...
                    UpdateGridPropVisibility( boGridPropertyIsHidden, hasVisibleChildren );
                }
                else
                {
                    UpdateGridPropVisibility( boGridPropertyIsHidden, false );
                }
            }
            else
            {
                UpdateGridPropVisibility( boGridPropertyIsHidden, boIsVisible );
            }
        }
    }
}
...
//-----------------------------------------------------------------------------
void PropData::UpdateGridPropVisibility( const bool boGridPropertyIsHidden, const bool boDriverPropertyIsVisible )
//-----------------------------------------------------------------------------
{
    if( boGridPropertyIsHidden && boDriverPropertyIsVisible )
    {
        m_pParentGridPage->HideProperty( m_GridItemId, false, wxPG_DONT_RECURSE );
    }
    else if( !boGridPropertyIsHidden && !boDriverPropertyIsVisible )
    {
...
MethodObject::MethodObject( HOBJ hObj ) : PropData( hObj ), m_Params( wxT( "" ) ),
    m_ParamDelimiter( wxT( " " ) ), m_FriendlyName( BuildFriendlyName( hObj ) )
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void MethodObject::UpdatePropData( wxString& /*errorMsg*/ )
//-----------------------------------------------------------------------------
{
    if( m_GridItemId )
    {
...
//-----------------------------------------------------------------------------
{
    ...
    else
    {
        ...
        if( m_pParentGridPage->GetPropertyTextColour( m_GridItemId ) != GlobalDataStorage::Instance()->GetPropGridColour( GlobalDataStorage::pgcContentDescriptorText ) )
        {
            m_GridItemId->SetTextColour( GlobalDataStorage::Instance()->GetPropGridColour( GlobalDataStorage::pgcContentDescriptorText ), wxPG_DONT_RECURSE );
        }
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    {
        wxString label( m_Title.empty() ? GetDisplayName( flags ) : m_Title );
        if( flags & dfDisplayDebugInfo )
        {
...
//-----------------------------------------------------------------------------
wxString PropertyObject::GetCurrentValueAsString( mvIMPACT::acquire::Component comp, ECtrl ctrlType, const int index /* = 0 */ )
//-----------------------------------------------------------------------------
{
    ...
    {
        return PropertyI( comp ).read( index ) == 0 ? wxT( "False" ) : wxT( "True" );
    }
    else if( ( representation == crIPv4Address ) && ( type == ctPropInt ) )
    {
        return GetIPv4AddressAsString( PropertyI( comp ), index );
    }
    else if( ( representation == crIPv4Address ) && ( type == ctPropInt64 ) )
    {
        return GetIPv4AddressAsString( PropertyI64( comp ), index );
    }
    else if( ( representation == crMACAddress ) && ( type == ctPropInt64 ) )
    {
        return MACAddressToString( PropertyI64( comp ).read( index ) );
    }
    else if( ( type == ctPropString ) && ( flags & cfContainsBinaryData ) )
    {
        ...
        return ConvertedString( BinaryDataToString( PropertyS( comp ).readBinary( index ) ) );
    }
    else
    {
        return ConvertedString( Property( comp ).readS( index, string( ( ctrlType == _ctrlMultiChoiceSelector ) ? "\"%s\" " : "" ) ) );
    }
}
...
//-----------------------------------------------------------------------------
void PropertyObject::WritePropVal( const string& value, mvIMPACT::acquire::Component comp, const int index /* = 0 */, const string& emptyString /* = "" */ )
//-----------------------------------------------------------------------------
{
    const TComponentType type( comp.type() );
    const TComponentRepresentation representation( comp.representation() );
    ...
    {
        PropertyI( comp ).write( ntohl( GetIPv4AddressFromString( value ) ), index );
    }
    else if( ( representation == crIPv4Address ) && ( type == ctPropInt64 ) )
    {
        PropertyI64( comp ).write( static_cast<int64_type>( ntohl( GetIPv4AddressFromString( value ) ) ), index );
    }
    else if( ( representation == crMACAddress ) && ( type == ctPropInt64 ) )
    {
        PropertyI64( comp ).write( MACAddressFromString( ConvertedString( value ) ), index );
    }
    else if( ( type == ctPropString ) && ( flags & cfContainsBinaryData ) )
    {
        // The 'type' check is only needed because some drivers with versions < 1.12.33
        // did incorrectly specify the 'cfContainsBinaryData' flag even though the data type was not 'ctPropString'...
        PropertyS( comp ).writeBinary( BinaryDataFromString( value ), index );
    }
    else
    {
        Property prop( comp );
        prop.writeS( ( ( flags & cfAllowValueCombinations ) && value.empty() ) ? emptyString : value, index );
    }
}
...
//-----------------------------------------------------------------------------
void PropertyObject::UpdatePropData( wxString& errorMsg )
//-----------------------------------------------------------------------------
{
    if( m_GridItemId )
    {
        ...
        {
            errorMsg = wxT( "Value too small! Clipping to minimum!" );
            try
            {
            ...
            {
                errorMsg.Append( wxString::Format( wxT( " Can't set value(Error: %s(%s))!" ), ConvertedString( e.getErrorString() ).c_str(), ConvertedString( e.getErrorCodeAsString() ).c_str() ) );
            }
        }
        catch( const EValTooLarge& )
        {
            errorMsg = wxT( "Value too large! Clipping to maximum!" );
            try
            {
        ...
        {
            errorMsg = wxString::Format( wxT( "This property can store %d value(s) only!" ), prop.valCount() );
        }
        catch( const EValidationFailed& )
        {
            errorMsg = wxT( "Value didn't pass validation check. The device log-file will contain additional information!" );
        }
        catch( const ImpactAcquireException& e )
        {
            errorMsg = wxString::Format( wxT( "Can't set value(Error: %s(%s))!" ), ConvertedString( e.getErrorString() ).c_str(), ConvertedString( e.getErrorCodeAsString() ).c_str() );
        }
        ...
        {
            errorMsg.Append( errorMsg.IsEmpty() ? wxT( "F" ) : wxT( " Also f" ) );
            errorMsg.Append( wxString::Format( wxT( "ailed to read value from property(Error: %s(%s))!" ), ConvertedString( e.getErrorString() ).c_str(), ConvertedString( e.getErrorCodeAsString() ).c_str() ) );
        }
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    else if( type == ctPropInt64 )
    {
        ...
        for( vector<pair<string, int64_type> >::size_type i = 0; i < vSize; i++ )
        {
            ...
            {
                soc.Add( ConvertedString( dict[i].first ), static_cast<int>( dict[i].second ) );
            }
        }
    }
...
//-----------------------------------------------------------------------------
{
    ...
    if( !m_GridItemId )
    {
        ...
        if( ( flags & dfSelectorGrouping ) && boIsSelector )
        {
            ...
            for( vector<mvIMPACT::acquire::Component>::size_type i = 0; i < cnt; i++ )
            {
                // the wxPropertyGrid does not support a category being the child of a non-category element
                if( selectedFeatures[i].type() != ctList )
                {
...
//-----------------------------------------------------------------------------
{
    const size_t propCnt = static_cast<size_t>( m_VectorItems.size() );
    wxWindowUpdateLocker updateLock( m_pParentGridPage->GetGrid() );
