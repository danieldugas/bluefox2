...
WizardMultiAOI::WizardMultiAOI( PropViewFrame* pParent, const wxString& title, mvIMPACT::acquire::Device* pDev, ImageCanvas* pImageCanvas, bool boAcquisitionStateOnCancel, FunctionInterface* pFI )
    : OkAndCancelDlg( pParent, widMainFrame, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxMINIMIZE_BOX ),
      ...
      AOIsForOverlay_(), previousSensorAOI_(), rectSensor_(), offsetXIncSensor_( -1LL ), offsetYIncSensor_( -1LL ), widthIncSensor_( -1LL ), heightIncSensor_( -1LL ),
...
//-----------------------------------------------------------------------------
{
    ...
    pAOIControls->pSetPanel = new wxPanel( pAOISettingsNotebook_, static_cast<wxWindowID>( widStaticBitmapWarning + value + 1 ), wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL, ConvertedString( ifc_.mvAreaSelector.readS() ) );
    ...
    const wxWindowID x_id = widLAST + static_cast<wxWindowID>( value ) * widSCAOIx;
    const wxWindowID y_id = widLAST + static_cast<wxWindowID>( value ) * widSCAOIy;
    const wxWindowID w_id = widLAST + static_cast<wxWindowID>( value ) * widSCAOIw;
    const wxWindowID h_id = widLAST + static_cast<wxWindowID>( value ) * widSCAOIh;
    ...
    pAOIControls->pSCAOIx = CreateAOISpinControl( pAOIControls->pSetPanel, x_id, 0, ifc_.widthMax.read(), ifc_.mvAreaOffsetX.read(), ifc_.offsetX.getStepWidth() );
    ...
    pAOIControls->pSCAOIy = CreateAOISpinControl( pAOIControls->pSetPanel, y_id, 0, ifc_.heightMax.read(), ifc_.mvAreaOffsetY.read(), ifc_.offsetY.getStepWidth() );
    ...
    pAOIControls->pSCAOIw = CreateAOISpinControl( pAOIControls->pSetPanel, w_id, ifc_.width.getMinValue(), ifc_.widthMax.read(), ifc_.mvAreaWidth.read(), ifc_.width.getStepWidth() );
    ...
    pAOIControls->pSCAOIh = CreateAOISpinControl( pAOIControls->pSetPanel, h_id, ifc_.height.getMinValue(), ifc_.heightMax.read(), ifc_.mvAreaHeight.read(), ifc_.height.getStepWidth() );
    ...
    if( boInsertOnFirstFreePosition )
    {
        ...
        pAOISettingsNotebook_->InsertPage( static_cast<size_t>( position ), pAOIControls->pSetPanel, pAOIControls->pSetPanel->GetName(), true );
    }
    else
    {
...
//-----------------------------------------------------------------------------
{
    try
    {
        ...
        // multi-AOI again as this wizard will override the current AOI with the maximum AOI in order to present
        // a full image to the user allowing him to move around the AOIs.
        if( ifc_.offsetX.isValid() && ifc_.offsetX.isWriteable() )
        {
            ifc_.offsetX.write( ifc_.offsetX.getMinValue() );
        }
        if( ifc_.offsetY.isValid() && ifc_.offsetY.isWriteable() )
        {
            ifc_.offsetY.write( ifc_.offsetY.getMinValue() );
        }
        ...
        while( it != itEND )
        {
            ...
            ifc_.mvAreaWidth.write( static_cast<int64_type>( it->second->pSCAOIw->GetValue() ) );
            ifc_.mvAreaHeight.write( static_cast<int64_type>( it->second->pSCAOIh->GetValue() ) );
            ifc_.mvAreaOffsetX.write( static_cast<int64_type>( it->second->pSCAOIx->GetValue() ) );
            ifc_.mvAreaOffsetY.write( static_cast<int64_type>( it->second->pSCAOIy->GetValue() ) );
            ++it;
        }
...
//-----------------------------------------------------------------------------
{
    ...
    const wxWindowID x_id = static_cast<wxWindowID>( widLAST + AOIIndex * widSCAOIx );
    const wxWindowID y_id = static_cast<wxWindowID>( widLAST + AOIIndex * widSCAOIy );
    const wxWindowID w_id = static_cast<wxWindowID>( widLAST + AOIIndex * widSCAOIw );
    const wxWindowID h_id = static_cast<wxWindowID>( widLAST + AOIIndex * widSCAOIh );
    ...
    pAOISettingsNotebook_->DeletePage( static_cast<size_t>( currentPosition ) );
...
//-----------------------------------------------------------------------------
{
    ...
    if( it != AOI2ControlsMap_.end() )
    {
        ...
        it->second->pSCAOIx->SetValue( ( p->m_rect.GetX() / p->m_incX ) * p->m_incX );
        it->second->pSCAOIy->SetValue( ( p->m_rect.GetY() / p->m_incY ) * p->m_incY );
        it->second->pSCAOIw->SetValue( ( p->m_rect.GetWidth() / p->m_incW ) * p->m_incW );
        it->second->pSCAOIh->SetValue( ( p->m_rect.GetHeight() / p->m_incH ) * p->m_incH );
...
//-----------------------------------------------------------------------------
{
    ...
    while( it != itEND )
    {
        const wxRect rect1( static_cast<int>( it->second->pSCAOIx->GetValue() ),
                            static_cast<int>( it->second->pSCAOIy->GetValue() ),
                            static_cast<int>( it->second->pSCAOIw->GetValue() ),
                            static_cast<int>( it->second->pSCAOIh->GetValue() ) );
