...
WizardLUTControl::WizardLUTControl( wxWindow* pParent, const wxString& title, mvIMPACT::acquire::GenICam::LUTControl& lc, const wxArrayString& lutSelectorEntries )
    : OkAndCancelDlg( pParent, widMainFrame, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxMAXIMIZE_BOX | wxMINIMIZE_BOX ),
      ...
      pBtnSynchronize_( 0 ), pBtnInvert_( 0 ), pBtnGamma_( 0 ), pBtnCopyTo_( 0 ), pBtnImport_( 0 ), pBtnExport_( 0 ), pLogWindow_( 0 ),
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    pBtnGamma_->SetToolTip( wxT( "Create data for the selected LUT from a gamma value. The following formula is used for the gamma LUT calculation: (1 + GammaAlpha)*inputValue^(Gamma) - GammaAlpha" ) );
...
//-----------------------------------------------------------------------------
{
    ...
    {
        const vector<int>::size_type srcLUTSize = lut.size();
        const vector<int>::size_type dstLUTSize = it->second.lut_.size();
        size_t destLUTIndex = 0;
        const string srcLUTidentifier( lc_.LUTSelector.readS() );

        const int64_type srcLutMaxVal = lc_.LUTValue.getMaxValue() + 1;
        // destination LUT
        if( lc_.LUTSelector.isValid() && lc_.LUTSelector.isWriteable() )
        {
            lc_.LUTSelector.writeS( lutIdentifier.ToStdString() );
        }
        const int64_type dstLutMaxVal = lc_.LUTValue.getMaxValue() + 1;
        if( lc_.LUTSelector.isValid() && lc_.LUTSelector.isWriteable() )
        {
            lc_.LUTSelector.writeS( srcLUTidentifier );
        }
        const double scaleFactor = static_cast<double>( dstLutMaxVal ) / static_cast<double>( srcLutMaxVal );

        if( ( dstLUTSize == srcLUTSize ) && ( dstLutMaxVal == srcLutMaxVal ) )
        {
            it->second.lut_ = lut;
        ...
        {
            if( dstLUTSize > srcLUTSize )
            {
                const size_t dstStepWidth = dstLUTSize / srcLUTSize;
                if( srcLutMaxVal > dstLutMaxVal )
                {
                    for( size_t i = 0; i < srcLUTSize; i++ )
                    {
                        const double delta = ( i < srcLUTSize - 1 ) ? static_cast<double>( lut[i + 1] - lut[i] ) / dstStepWidth : 0.;
                        for( size_t j = 0; j < dstStepWidth; j++ )
                        {
                            it->second.lut_[destLUTIndex] = static_cast<int>( ( lut[i] + ( delta * j ) ) * scaleFactor );
                            ++destLUTIndex;
                        }
                    }
                }
                else if( srcLutMaxVal < dstLutMaxVal )
                {
                    for( size_t i = 0; i < srcLUTSize; i++ )
                    {
                        const double delta = ( i < srcLUTSize - 1 ) ? static_cast<double>( lut[i + 1] - lut[i] ) / dstStepWidth : 0.;
                ...
                {
                    for( size_t i = 0; i < srcLUTSize - 1 ; i++ )
                    {
                        const double delta = ( i < srcLUTSize - 1 ) ? static_cast<double>( lut[i + 1] - lut[i] ) / dstStepWidth : 0.;
                        for( size_t j = 0; j < dstStepWidth; j++ )
                        {
            ...
            else if( dstLUTSize < srcLUTSize )
            {
                const size_t srcStepWidth = srcLUTSize / dstLUTSize;
                if( srcLutMaxVal > dstLutMaxVal )
                {
                    for( size_t i = 0; i < dstLUTSize - 1 ; i++ )
                    {
                        it->second.lut_[destLUTIndex] = static_cast<int>( lut[i * srcStepWidth] / scaleFactor );
                        ++destLUTIndex;
                    }
                }
                else
                {
                    for( size_t i = 0; i < dstLUTSize  ; i++ )
                    {
                        it->second.lut_[destLUTIndex] = static_cast<int>( lut[i * srcStepWidth] * scaleFactor );
                        ++destLUTIndex;
                    }
            ...
            else
            {
                for( size_t i = 0; i < dstLUTSize ; i++ )
                {
                    it->second.lut_[destLUTIndex] = static_cast<int>( lut[i] * scaleFactor );
                    ++destLUTIndex;
                }
            }
        }
...
//-----------------------------------------------------------------------------
{
    try
    {
        ...
        lut.resize( static_cast<vector<int>::size_type>( lc_.LUTIndex.getMaxValue() + 1 ) );
        const vector<int>::size_type vSize = lut.size();
        int transformedValue = 0;

        double gamma = data.gamma_;
...
//-----------------------------------------------------------------------------
{
    ...
    if( fileDlg.ShowModal() == wxID_OK )
    {
        ...
        for( size_t i = 1; i < elementsToProcess; i++ )
        {
            if( file.Eof() )
            {
                wxString msg;
        ...
        {
            wxArrayString lineTokens = wxSplit( lines[i], wxT( ';' ) );
...
//-----------------------------------------------------------------------------
void WizardLUTControl::OnLUTSelectionChanged( void )
//-----------------------------------------------------------------------------
{
    SetupLUTEnable();
    LUTMap::iterator it = lutMap_.find( pCBLUTSelection_->GetValue() );
...
//-----------------------------------------------------------------------------
{
    ...
    try
    {
        ...
        const int64_type lutSize = lc_.LUTIndex.getMaxValue() + 1;
        ...
        wxProgressDialog dlg( wxString::Format( wxT( "Reading LUT '%s' from device" ), LUTSelectorValue.c_str() ),
                              wxString::Format( wxT( "Reading LUT '%s' from device(%05d/%05d elements read)" ), LUTSelectorValue.c_str(), 0, static_cast<int>( lutSize ) ),
                              static_cast<int>( lutSize ), // range
                              this,     // parent
                              wxPD_AUTO_HIDE | wxPD_APP_MODAL | wxPD_ELAPSED_TIME | wxPD_ESTIMATED_TIME | wxPD_REMAINING_TIME );

        for( int64_type i = 0; i < lutSize; i++ )
        {
            lc_.LUTIndex.write( i );
            lut[static_cast<vector<int>::size_type>( i )] = static_cast<int>( lc_.LUTValue.read() );
            if( i % 50 == 0 )
            {
                dlg.Update( static_cast<int>( i ), wxString::Format( wxT( "Reading LUT '%s' from device(%05d/%05d elements read)" ), LUTSelectorValue.c_str(), static_cast<int>( i ), static_cast<int>( lutSize ) ) );
            }
        }
...
//-----------------------------------------------------------------------------
{
    try
    {
        if( lc_.LUTSelector.isValid() && lc_.LUTSelector.isWriteable() )
        {
            ...
            pCBEnable_->Enable( lc_.LUTEnable.isWriteable() );
        }
...
//-----------------------------------------------------------------------------
{
    ...
    case dmGraphical:
        try
        {
            // visualization of interpolated LUTs needs to be handled in a different manner
            if( lc_.mvLUTType.isValid() && ( lc_.mvLUTType.readS() == "Interpolated" ) )
            {
                const int64_type displayLUTSize = static_cast<int64_type>( 1 << ( lc_.mvLUTMapping.read() >> 16 ) );
                const int64_type sourceLUTSize = lc_.LUTIndex.getMaxValue() + 1;
                const double lutInterpolationFactor = static_cast<double>( displayLUTSize ) / ( sourceLUTSize - 1 );
                vector<int> lutForDisplay( static_cast<vector<int>::size_type>( displayLUTSize ) );
                pGraphicalDisplay_->RefreshData( activeLUT_, static_cast<int>( displayLUTSize ) );
                if( !activeLUT_.empty() )
                {
                    int displayLUTIndex = 0;
                    // to show the characteristic curve for the full LUT width it is necessary to interpolate missing data points for displaying
                    for( int i = 0; i < sourceLUTSize - 1; i++ )
                    {
                        // calculate the elevation between the current sampling point and the next one and scale the result
                        // according to the number of data points between both in the target diagram of the characteristic curve
                        const double delta = static_cast<double>( activeLUT_[i + 1] - activeLUT_[i] ) / lutInterpolationFactor;
                        const int maxDisplayLutIndex = static_cast<int>( ( i + 1 ) * lutInterpolationFactor );
                        int j = 0;
                        while( displayLUTIndex < maxDisplayLutIndex )
                        {
                            // insert the resulting values in the LUT which should be used for the visualization in the LUT Wizard
                            lutForDisplay[static_cast<vector<int>::size_type>( displayLUTIndex )] = static_cast<int>(  activeLUT_[i] + ( round( delta ) * j ) );
                            displayLUTIndex++;
                            j++;
                        }
                    }
                    pGraphicalDisplay_->RefreshData( lutForDisplay, static_cast<int>( displayLUTSize ) );
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    pGridSizer->Add( new wxStaticText( pPanel, wxID_ANY, wxT( " (1 + GammaAlpha)*inputValue^(Gamma) - GammaAlpha " ) ), wxSizerFlags().Expand() );
