...
BEGIN_EVENT_TABLE( WizardColorCorrection, OkAndCancelDlg )
    ...
    EVT_COMMAND_SCROLL( widSLSaturation, WizardColorCorrection::OnSLSaturation )
    ...
    EVT_CLOSE( WizardColorCorrection::OnClose )
END_EVENT_TABLE()

//-----------------------------------------------------------------------------
WizardColorCorrection::WizardColorCorrection( PropViewFrame* pParent, const wxString& title, mvIMPACT::acquire::Device* pDev, mvIMPACT::acquire::FunctionInterface* pFI )
    : OkAndCancelDlg( pParent, widMainFrame, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxMAXIMIZE_BOX | wxMINIMIZE_BOX ),
      pDev_( pDev ), pFI_( pFI ), ip_( pDev ), pctc_( 0 ), pParent_( pParent ), boAcquisitionStateOnCancel_( false ),
      pCBEnableInputCorrectionMatrix_( 0 ), pInputCorrectionCombo_( 0 ), pCBEnableOutputCorrectionMatrix_( 0 ),
...
//-----------------------------------------------------------------------------
{
    if( WriteToDevice() )
    {
        ...
        {
            wxMessageBox( wxString::Format( wxT( "Failed to modify driver properties(Error: %s(%s))" ), ConvertedString( e.getErrorString() ).c_str(), ConvertedString( e.getErrorCodeAsString() ).c_str() ), wxT( "Failed To Write To Device" ), wxOK | wxICON_INFORMATION, this );
        }
...
//-----------------------------------------------------------------------------
void WizardColorCorrection::OnClose( wxCloseEvent& e )
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void WizardColorCorrection::CloseDlg( int result )
//-----------------------------------------------------------------------------
{
    if( result != wxID_OK )
    {
...
//-----------------------------------------------------------------------------
{
    ...
    {
        wxMessageBox( wxString::Format( wxT( "Failed To Write To Device(Error: %s(%s))" ), ConvertedString( e.getErrorString() ).c_str(), ConvertedString( e.getErrorCodeAsString() ).c_str() ), wxT( "Failed To Write To Device" ), wxOK | wxICON_INFORMATION, this );
    }
}
