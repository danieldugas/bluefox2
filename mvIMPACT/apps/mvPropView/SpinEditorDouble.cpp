        ...
        case ctPropInt64:
            {
                ...
                format = ConvertedString( prop.stringFormatString() );
                min = static_cast<double>( prop.hasMinValue() ? prop.getMinValue() : numeric_limits<int64_type>::min() );
                max = static_cast<double>( prop.hasMaxValue() ? prop.getMaxValue() : numeric_limits<int64_type>::max() );
                step = prop.hasStepWidth() ? prop.getStepWidth() : 1.0;
                mode = mInt64;
            }
            break;
