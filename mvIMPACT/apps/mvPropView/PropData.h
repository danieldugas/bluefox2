...
public:
    ...
    virtual void                        UpdatePropData( wxString& /*errorMsg*/ ) {}
protected:
    void                                AppendPropertyToGrid( wxPGProperty* pProp, wxPGProperty* pParentItem );
    void                                UpdateGridPropVisibility( const bool boGridPropertyIsHidden, const bool boDriverPropertyIsVisible );
...
private:
    ...
    static bool                         IsVisible( Component c );
    bool                                IsVisible( void ) const
    {
        return IsVisible( m_Component );
    }
...
public:
    ...
    const wxString&                     ParamDelimiter( void ) const
    {
        return m_ParamDelimiter;
    }
    void                                SetParamDelimiter( const wxString& delimiter )
    {
        m_ParamDelimiter = delimiter;
    }
    void                                UpdatePropData( wxString& errorMsg );
...
public:
    ...
    virtual wxString                    BuildFullFeaturePath( void ) const;
    void                                EnsureValidGridItem( const PropTree* pPropTree, wxPGProperty* pParentGridComponent, EDisplayFlags flags, bool* pboModified = 0 );
    static wxString                     GetCurrentValueAsString( mvIMPACT::acquire::Component comp, const int index = 0 )
    {
        const wxString elementName( comp.name() );
        return PropertyObject::GetCurrentValueAsString( comp, PropertyObject::GetEditorType( comp, elementName ), index );
    }
    static wxString                     GetCurrentValueAsString( mvIMPACT::acquire::Component comp, ECtrl ctrlType, const int index = 0 );
    ...
    void                                Update( const PropTree* pPropTree, EDisplayFlags flags, unsigned int actChangedCount, unsigned int actAttrChangedCount ) const;
    void                                UpdatePropData( wxString& errorMsg );
    static void                         WritePropVal( const std::string& value, mvIMPACT::acquire::Component comp, const int index = 0, const std::string& emptyString = "" );
private:
    wxString                            GetCurrentValueAsString( void ) const
    {
        return GetCurrentValueAsString( GetComponent(), m_Type, m_Index );
    }
    ...
    {
        WritePropVal( value, GetComponent(), index, m_emptyString );
    }

    const int                           m_Index;
    const bool                          m_boVectorAsList;
    mutable std::string                 m_emptyString; // The string that is used for multi-choice editors when none of the choices has been selected
};
