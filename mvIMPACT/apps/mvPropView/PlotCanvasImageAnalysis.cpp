...
//-----------------------------------------------------------------------------
PlotCanvasImageAnalysis::~PlotCanvasImageAnalysis()
//-----------------------------------------------------------------------------
{
    DeleteElement( m_pAOI );
}
...
//-----------------------------------------------------------------------------
{
    ...
    if( m_pImageCanvas )
    {
        if( boActive )
        {
            m_pAOI = new AOI( wxRect( m_AOIx, m_AOIy, m_AOIw, m_AOIh ), m_AOIColour, GetName() );
            m_pImageCanvas->RegisterAOI( m_pAOI );
        }
        else
        {
            m_pImageCanvas->UnregisterAOI( m_pAOI );
            DeleteElement( m_pAOI );
        }
    }
}
...
//-----------------------------------------------------------------------------
{
    if( ( value > 0 ) && AssignDisplayParameter( m_HistoryDepth, value ) )
    {
        UpdateInternalData();
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    if( m_pImageCanvas && m_pAOI && boChanged )
    {
        m_pImageCanvas->UpdateAOI( m_pAOI, m_AOIx, m_AOIy, m_AOIw, m_AOIh );
    }
