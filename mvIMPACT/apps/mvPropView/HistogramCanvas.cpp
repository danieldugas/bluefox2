            ...
            if( m_bayerParity == bmpUndefined )
            {
                for( int line = 0; line < m_AOIh; line++ )
                {
                    ...
                    for( int pixel = 0; pixel < m_AOIw; pixel++ )
                    {
                        if( pixel < m_AOIw - 1 )
                        {
                            const unsigned char difference = static_cast<unsigned char>( abs( *pData - * ( pData + 1 ) ) );
                            m_pHistogramAverage[pChannel0Hor] += difference;
                            ++m_ppHistogramBuffer[pChannel0Hor][difference];
                        }
                        if( line < m_AOIh - 1 )
                        {
                            const unsigned char difference = static_cast<unsigned char>( abs( *pData - * ( pData + pIB->pChannels[0].iLinePitch ) ) );
            ...
            else
            {
                for( int line = 0; line < m_AOIh; line++ )
                {
                    ...
                    for( int pixel = 0; pixel < m_AOIw; pixel++ )
                    {
                        ...
                        if( pixel < m_AOIw - 2 )
                        {
                            const unsigned char difference = static_cast<unsigned char>( abs( *pData - * ( pData + 2 ) ) );
                            m_pHistogramAverage[2 * index] += difference;
                            ++m_ppHistogramBuffer[2 * index][difference];
                        }
                        if( line < m_AOIh - 2 )
                        {
                            const unsigned char difference = static_cast<unsigned char>( abs( *pData - * ( pData + 2 * pIB->pChannels[0].iLinePitch ) ) );
            ...
            if( m_bayerParity == bmpUndefined )
            {
                for( int line = 0; line < m_AOIh; line++ )
                {
                    ...
                    for( int pixel = 0; pixel < m_AOIw; pixel++ )
                    {
                        ...
                        if( pixel < m_AOIw - 1 )
                        {
                            const unsigned short next_pixel_right = saveAssign( *( pData + 1 ), static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) );
                            const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_right ) );
                            m_pHistogramAverage[pChannel0Hor] += difference;
                            ++m_ppHistogramBuffer[pChannel0Hor][difference];
                        }
                        if( line < m_AOIh - 1 )
                        {
                            const unsigned short next_pixel_down = saveAssign( *reinterpret_cast<unsigned short*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ), static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) );
                            const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_down ) );
                            m_pHistogramAverage[pChannel0Ver] += difference;
                            ++m_ppHistogramBuffer[pChannel0Ver][difference];
                        }
                        ++pData;
            ...
            else
            {
                for( int line = 0; line < m_AOIh; line++ )
                {
                    ...
                    for( int pixel = 0; pixel < m_AOIw; pixel++ )
                    {
                        ...
                        if( pixel < m_AOIw - 2 )
                        {
                            const unsigned short next_pixel_right = saveAssign( *( pData + 2 ), static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) );
                            const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_right ) );
                            m_pHistogramAverage[2 * index] += difference;
                            ++m_ppHistogramBuffer[2 * index][difference];
                        }
                        if( line < m_AOIh - 2 )
                        {
                            const unsigned short next_pixel_down = saveAssign( *reinterpret_cast<unsigned short*>( reinterpret_cast<unsigned char*>( pData ) + 2 * pIB->pChannels[0].iLinePitch ), static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) );
                            const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_down ) );
                            m_pHistogramAverage[2 * index + 1] += difference;
                            ++m_ppHistogramBuffer[2 * index + 1][difference];
                        }
            ...
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    ...
                    if( pixel < m_AOIw - 1 )
                    {
                        ...
                        difference = static_cast<unsigned short>( abs( red0 - red1 ) );
                        m_pHistogramAverage[pChannel0Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel0Hor][difference];
                        difference = static_cast<unsigned short>( abs( green0 - green1 ) );
                        m_pHistogramAverage[pChannel1Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel1Hor][difference];
                        difference = static_cast<unsigned short>( abs( blue0 - blue1 ) );
    ...
    case ibpfYUV444_10Packed:
        {
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    ...
                    if( pixel < m_AOIw - 1 )
                    {
                        difference = static_cast<unsigned short>( abs( saveAssign( pData[0], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) - saveAssign( pData[0 + pIB->iChannelCount], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) ) );
                        m_pHistogramAverage[pChannel0Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel0Hor][difference];
                        difference = static_cast<unsigned short>( abs( saveAssign( pData[1], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) - saveAssign( pData[1 + pIB->iChannelCount], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) ) );
                        m_pHistogramAverage[pChannel1Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel1Hor][difference];
                        difference = static_cast<unsigned short>( abs( saveAssign( pData[2], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) - saveAssign( pData[2 + pIB->iChannelCount], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) ) );
                        m_pHistogramAverage[pChannel2Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel2Hor][difference];
                    }
                    if( line < m_AOIh - 1 )
                    {
                        ...
                        difference = static_cast<unsigned short>( abs( saveAssign( pData[0], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) - saveAssign( pNext_pixel_down[0], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) ) );
                        m_pHistogramAverage[pChannel0Ver] += difference;
                        ++m_ppHistogramBuffer[pChannel0Ver][difference];
                        difference = static_cast<unsigned short>( abs( saveAssign( pData[1], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) - saveAssign( pNext_pixel_down[1], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) ) );
                        m_pHistogramAverage[pChannel1Ver] += difference;
                        ++m_ppHistogramBuffer[pChannel1Ver][difference];
                        difference = static_cast<unsigned short>( abs( saveAssign( pData[2], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) - saveAssign( pNext_pixel_down[2], static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) ) ) );
                        m_pHistogramAverage[pChannel2Ver] += difference;
                        ++m_ppHistogramBuffer[pChannel2Ver][difference];
                    }
    ...
    case ibpfRGBx888Planar:
        {
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    if( pixel < m_AOIw - 1 )
                    {
                        difference = static_cast<unsigned char>( abs( pr[0] - pr[1] ) );
                        m_pHistogramAverage[pChannel0Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel0Hor][difference];
                        difference = static_cast<unsigned char>( abs( pg[0] - pg[1] ) );
                        m_pHistogramAverage[pChannel1Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel1Hor][difference];
                        difference = static_cast<unsigned char>( abs( pb[0] - pb[1] ) );
                        m_pHistogramAverage[pChannel2Hor] += difference;
                        ++m_ppHistogramBuffer[pChannel2Hor][difference];
                    }
                    if( line < m_AOIh - 1 )
                    {
                        difference = static_cast<unsigned char>( abs( pr[0] - pr[pIB->pChannels[0].iLinePitch] ) );
                        m_pHistogramAverage[pChannel0Ver] += difference;
                        ++m_ppHistogramBuffer[pChannel0Ver][difference];
                        difference = static_cast<unsigned char>( abs( pg[0] - pg[pIB->pChannels[1].iLinePitch] ) );
                        m_pHistogramAverage[pChannel1Ver] += difference;
                        ++m_ppHistogramBuffer[pChannel1Ver][difference];
                        difference = static_cast<unsigned char>( abs( pb[0] - pb[pIB->pChannels[2].iLinePitch] ) );
                        m_pHistogramAverage[pChannel2Ver] += difference;
                        ++m_ppHistogramBuffer[pChannel2Ver][difference];
                    }
                    ++pr;
    ...
    case ibpfYUV422Planar:
        for( int line = 0; line < m_AOIh; line++ )
        {
            ...
            for( int pixel = 0; pixel < m_AOIw; pixel++ )
            {
                if( pixel < m_AOIw - 1 )
                {
                    difference = static_cast<unsigned char>( abs( py[0] - py[1] ) );
                    m_pHistogramAverage[pChannel0Hor] += difference;
                    ++m_ppHistogramBuffer[pChannel0Hor][difference];
                    difference = static_cast<unsigned char>( abs( pu[0] - pu[1] ) );
                    m_pHistogramAverage[pChannel1Hor] += difference;
                    ++m_ppHistogramBuffer[pChannel1Hor][difference];
                    difference = static_cast<unsigned char>( abs( pv[0] - pv[1] ) );
                    m_pHistogramAverage[pChannel2Hor] += difference;
                    ++m_ppHistogramBuffer[pChannel2Hor][difference];
                }
                if( line < m_AOIh - 1 )
                {
                    difference = static_cast<unsigned char>( abs( py[0] - py[pIB->pChannels[0].iLinePitch] ) );
                    m_pHistogramAverage[pChannel0Ver] += difference;
                    ++m_ppHistogramBuffer[pChannel0Ver][difference];
                    difference = static_cast<unsigned char>( abs( pu[0] - pu[pIB->pChannels[1].iLinePitch] ) );
                    m_pHistogramAverage[pChannel1Ver] += difference;
                    ++m_ppHistogramBuffer[pChannel1Ver][difference];
                    difference = static_cast<unsigned char>( abs( pv[0] - pv[pIB->pChannels[2].iLinePitch] ) );
                    m_pHistogramAverage[pChannel2Ver] += difference;
                    ++m_ppHistogramBuffer[pChannel2Ver][difference];
                }
                ++py;
...
//-----------------------------------------------------------------------------
void HistogramCanvas::CalculateSpatialNoiseHistogram_8u_C3Packed( const ImageBuffer* pIB, const int order[3] )
//-----------------------------------------------------------------------------
{
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = 0; pixel < m_AOIw; pixel++ )
        {
            if( pixel < m_AOIw - 1 )
            {
                difference = static_cast<unsigned char>( abs( pData[order[0]] - pData[order[0] + pIB->iBytesPerPixel] ) );
                m_pHistogramAverage[pChannel0Hor] += difference;
                ++m_ppHistogramBuffer[pChannel0Hor][difference];
                difference = static_cast<unsigned char>( abs( pData[order[1]] - pData[order[1] + pIB->iBytesPerPixel] ) );
                m_pHistogramAverage[pChannel1Hor] += difference;
                ++m_ppHistogramBuffer[pChannel1Hor][difference];
                difference = static_cast<unsigned char>( abs( pData[order[2]] - pData[order[2] + pIB->iBytesPerPixel] ) );
                m_pHistogramAverage[pChannel2Hor] += difference;
                ++m_ppHistogramBuffer[pChannel2Hor][difference];
            }
            if( line < m_AOIh - 1 )
            {
                difference = static_cast<unsigned char>( abs( pData[order[0]] - pData[order[0] + pIB->pChannels[0].iLinePitch] ) );
                m_pHistogramAverage[pChannel0Ver] += difference;
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = static_cast<unsigned char>( abs( pData[order[1]] - pData[order[1] + pIB->pChannels[0].iLinePitch] ) );
                m_pHistogramAverage[pChannel1Ver] += difference;
                ++m_ppHistogramBuffer[pChannel1Ver][difference];
                difference = static_cast<unsigned char>( abs( pData[order[2]] - pData[order[2] + pIB->pChannels[0].iLinePitch] ) );
                m_pHistogramAverage[pChannel2Ver] += difference;
                ++m_ppHistogramBuffer[pChannel2Ver][difference];
            }
...
//-----------------------------------------------------------------------------
{
    ...
    if( m_bayerParity == bmpUndefined )
    {
        for( int line = 0; line < m_AOIh; line++ )
        {
            ...
            for( int pixel = 0; pixel < m_AOIw; pixel++ )
            {
                const unsigned short current_pixel = pixelAccessFn( pData, pixOffset + m_AOIx + pixel );
                if( pixel < m_AOIw - 1 )
                {
                    const unsigned short next_pixel_right = pixelAccessFn( pData, pixOffset + m_AOIx + pixel + 1 );
                    const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_right ) );
                    m_pHistogramAverage[pChannel0Hor] += difference;
                    ++m_ppHistogramBuffer[pChannel0Hor][difference];
                }
                if( line < m_AOIh - 1 )
                {
                    const unsigned short next_pixel_down = pixelAccessFn( pData, pixOffset + pIB->iWidth + m_AOIx + pixel );
                    const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_down ) );
                    m_pHistogramAverage[pChannel0Ver] += difference;
                    ++m_ppHistogramBuffer[pChannel0Ver][difference];
                }
    ...
    else
    {
        for( int line = 0; line < m_AOIh; line++ )
        {
            for( int pixel = 0; pixel < m_AOIw; pixel++ )
            {
                ...
                if( pixel < m_AOIw - 2 )
                {
                    const unsigned short next_pixel_right = pixelAccessFn( pData, pixOffset + m_AOIx + pixel + 2 );
                    const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_right ) );
                    m_pHistogramAverage[2 * index] += difference;
                    ++m_ppHistogramBuffer[2 * index][difference];
                }
                if( line < m_AOIh - 2 )
                {
                    const unsigned short next_pixel_down = pixelAccessFn( pData, pixOffset + 2 * pIB->iWidth + m_AOIx + pixel );
                    const unsigned short difference = static_cast<unsigned short>( abs( current_pixel - next_pixel_down ) );
                    m_pHistogramAverage[2 * index + 1] += difference;
                    ++m_ppHistogramBuffer[2 * index + 1][difference];
                }
...
//-----------------------------------------------------------------------------
{
    ...
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = 0; pixel < m_AOIw / 2; pixel++ )
        {
            if( pixel < ( m_AOIw - 3 ) / 2 )
            {
                difference = static_cast<_Ty>( abs( pData[1] - pData[3] ) );
                m_pHistogramAverage[pChannel0Hor] += difference;
                ++m_ppHistogramBuffer[pChannel0Hor][difference];
                difference = static_cast<_Ty>( abs( pData[3] - pData[5] ) );
                m_pHistogramAverage[pChannel0Hor] += difference;
                ++m_ppHistogramBuffer[pChannel0Hor][difference];
                difference = static_cast<_Ty>( abs( pData[0] - pData[4] ) );
                ...
                difference = static_cast<_Ty>( abs( pData[2] - pData[6] ) );
            ...
            if( line < m_AOIh - 1 )
            {
                difference = static_cast<_Ty>( abs( pData[1] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + 1 ) ) );
                m_pHistogramAverage[pChannel0Ver] += difference;
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = static_cast<_Ty>( abs( pData[0] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + 0 ) ) );
                ...
                difference = static_cast<_Ty>( abs( pData[3] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + 3 ) ) );
                m_pHistogramAverage[pChannel0Ver] += difference;
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = static_cast<_Ty>( abs( pData[2] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + 2 ) ) );
...
//-----------------------------------------------------------------------------
{
    ...
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = m_AOIx; pixel < LOOP_END; pixel++ )
        {
            ...
            if( pixel < LOOP_END - 1 )
            {
                difference = static_cast<_Ty>( abs( pData[0] - pData[incToNextYComponent] ) );
                m_pHistogramAverage[pChannel1Hor] += difference;
                ++m_ppHistogramBuffer[pChannel1Hor][difference];
                if( pixel < LOOP_END - 4 )
                {
                    difference = static_cast<_Ty>( abs( pData[ph.GetUOffset()] - pData[ph.GetUOffset() + 6] ) );
                    m_pHistogramAverage[pChannel0Hor] += difference;
                    ++m_ppHistogramBuffer[pChannel0Hor][difference];
                    difference = static_cast<_Ty>( abs( pData[ph.GetVOffset()] - pData[ph.GetVOffset() + 6] ) );
                    m_pHistogramAverage[pChannel2Hor] += difference;
                    ++m_ppHistogramBuffer[pChannel2Hor][difference];
                }
            }
            if( line < m_AOIh - 1 )
            {
                ...
                m_pHistogramAverage[pChannel1Ver] += difference;
                ++m_ppHistogramBuffer[pChannel1Ver][difference];
                difference = static_cast<_Ty>( abs( pData[ph.GetUOffset()] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + ph.GetUOffset() ) ) );
                m_pHistogramAverage[pChannel0Ver] += difference;
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = static_cast<_Ty>( abs( pData[ph.GetVOffset()] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + ph.GetVOffset() ) ) );
                m_pHistogramAverage[pChannel2Ver] += difference;
                ++m_ppHistogramBuffer[pChannel2Ver][difference];
            }
...
//-----------------------------------------------------------------------------
{
    ...
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = 0; pixel < m_AOIw / 2; pixel++ )
        {
            if( pixel < ( m_AOIw - 3 ) / 2 )
            {
                difference = static_cast<_Ty>( abs( pData[0] - pData[2] ) );
                m_pHistogramAverage[pChannel0Hor] += difference;
                ++m_ppHistogramBuffer[pChannel0Hor][difference];
                difference = static_cast<_Ty>( abs( pData[2] - pData[4] ) );
                m_pHistogramAverage[pChannel0Hor] += difference;
                ++m_ppHistogramBuffer[pChannel0Hor][difference];
                difference = static_cast<_Ty>( abs( pData[1] - pData[5] ) );
                ...
                difference = static_cast<_Ty>( abs( pData[3] - pData[7] ) );
            ...
            if( line < m_AOIh - 1 )
            {
                ...
                m_pHistogramAverage[pChannel0Ver] += difference;
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = static_cast<_Ty>( abs( pData[3] - * ( reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pData ) + pIB->pChannels[0].iLinePitch ) + 1 ) ) );
    ...
    case ibpfMono8:
        ...
        else
        {
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    const int index = ( ( ( line + m_AOIy ) & 1 ) ? ( ( ( pixel + m_AOIx ) & 1 ) ? pOddOdd : pOddEven ) : ( ( ( pixel + m_AOIx ) & 1 ) ? pEvenOdd : pEvenEven ) );
    ...
    case ibpfMono16:
        if( m_bayerParity == bmpUndefined )
        {
            for( int line = 0; line < m_AOIh; line++ )
            {
                ...
                for( int pixel = 0; pixel < m_AOIw; pixel++ )
                {
                    const unsigned short value = saveAssign( *pData, static_cast<unsigned short>( 0 ), static_cast<unsigned short>( m_valCount - 1 ) );
...
//-----------------------------------------------------------------------------
void HistogramCanvas::CalculatePixelHistogram_8u_RGBPacked( const ImageBuffer* pIB, const int order[3] )
//-----------------------------------------------------------------------------
{
    for( int line = 0; line < m_AOIh; line++ )
    {
...
//-----------------------------------------------------------------------------
{
    if( m_bayerParity == bmpUndefined )
    {
        for( int line = 0; line < m_AOIh; line++ )
        {
            ...
            for( int pixel = 0; pixel < m_AOIw; pixel++ )
            {
                const unsigned short value = pixelAccessFn( static_cast<unsigned char*>( pIB->vpData ), pixOffset + m_AOIx + pixel );
                m_pHistogramAverage[pRed] += value;
                ++m_ppHistogramBuffer[pRed][value];
            }
        }
...
//-----------------------------------------------------------------------------
{
    const int lowerStart = h - GetBorderWidth();
